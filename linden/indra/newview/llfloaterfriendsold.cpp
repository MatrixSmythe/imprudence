...
// static
BOOL LLFloaterFriends::visible()
{
	return sInstance && sInstance->getVisible();
}

// static
void LLFloaterFriends::toggle()
{
	if (sInstance)
	{
		sInstance->close();
...
BOOL LLFloaterFriends::updateFriendItem(const LLUUID& agent_id, const LLRelationship* info)
{
	...
	itemp->getColumn(LIST_VISIBLE_MAP_THEIRS)->setValue(statusIcon);
		...
		if(ids.size() == 1)
		{
			...
			if (command == GRANT)
			{
				LLNotifications::instance().add("GrantModifyRights", 
					args, 
					LLSD(), 
					boost::bind(&LLFloaterFriends::modifyRightsConfirmation, this, _1, _2, rights));
			}
			else
			{
...
bool LLFloaterFriends::modifyRightsConfirmation(const LLSD& notification, const LLSD& response, rights_map_t* rights)
{
	S32 option = LLNotification::getSelectedOption(notification, response);
	if(0 == option)
	{
